...
torchrun --standalone --nnodes=1 --nproc-per-node=$NUM_GPUS python metrics.py \
    --mode test \
    --model_paths ${EXPE_NAME}

torchrun --standalone --nnodes=1 --nproc-per-node=2 metrics.py \
    --mode test \
    --model_paths ./output/small_aerial_500kit_200kdensify_n1d8/ 
